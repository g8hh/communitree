		...
		template: `
			<div class="upgRow">
				<div v-for="tab in Object.keys(data)">
					...
					v-bind:style="[{'background-color': tmp[layer].color}, (data[tab].glowColor && subtabShouldNotify(layer, name, tab) ? {'box-shadow': 'var(--hqProperty2a), 0 0 20px '  + data[tab].glowColor} : {}), tmp[layer].componentStyles['tab-button'], data[tab].buttonStyle]"
						v-on:click="function(){player.subtabs[layer][name] = tab; updateTabFormats(); needCanvasUpdate = true;}">{{run(data[tab].title) || tab}}</button>
				</div>
			</div>
		`
	},
      ...
      v-if="tmp[layer].tooltip != ''"
			:text="(tmp[layer].isLayer) ? (
				player[layer].unlocked ? (tmp[layer].tooltip ? tmp[layer].tooltip : formatWhole(player[layer].points) + ' ' + tmp[layer].resource +
					(player[layer].best.gte(1) ? '<br/><span>' + formatTime(player[layer].resetTime) + '</span>' : ''))
    ...
    'options-tab': {
        ...
        <table>
            ...
            <tr>
                ...
                <td><button class="opt" onclick="toggleOpt('inlineExp')">In-line Exponent: {{ player.inlineExp?"ON":"OFF" }}</button></td>
			</tr> 
            <tr>
				<td><button class="opt" onclick="toggleOpt('antiEpilepsy')">Anti-Epilepsy Mode: {{ player.antiEpilepsy?"ON":"OFF" }}</button></td>
			</tr> 
        </table>`
    },
